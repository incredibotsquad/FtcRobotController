...
public class IntakeTest extends LinearOpMode {
    ...
    private Boolean isTeamColor = false;
            ...
            if (DoingSpecimen) {
                if (prox <= proxnum) {
                    if (teamColor.equals("BLUE")) {
                        ...
                        isTeamColor = (colorSensor.blue() > colorSensor.red() && colorSensor.blue() > colorSensor.green());
                    } else {
                        telemetry.addData("Checking for team color: ", "RED OR YELLOW");
                        isTeamColor = (colorSensor.red() > colorSensor.blue() && colorSensor.red() > colorSensor.green());
                    }
                }
                if (!isTeamColor) {
                    Outtake();
    ...
    private void Outtake() {
        ...
        counter = 0;
        while (counter <= 50000) {
            telemetry.addData("In taking teancolor: ", isTeamColor);
