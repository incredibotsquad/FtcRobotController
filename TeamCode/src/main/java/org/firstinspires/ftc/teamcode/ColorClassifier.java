...
//
//    // ----- Tunable thresholds -----
//    // Brightness sanity (ignore ultra-dim noise)
//    private static final double MIN_CONFIDENCE_V = 0.006;
//
//    // PURPLE (blue-violet)
//    private static final double PURPLE_H_MIN = 220.0;
//    private static final double PURPLE_H_MAX = 240.0;
//    private static final double PURPLE_MIN_S = 0.42;
//    private static final double PURPLE_MIN_PURPLE_STR = 0.80; // b/(r+g)
//
//    // GREEN
//    private static final double GREEN_H_MIN = 158.0;
//    private static final double GREEN_H_MAX = 175.0;
//    private static final double GREEN_MIN_S  = 0.62;
//    private static final double GREEN_MIN_GREEN_STRENGTH = 0.85; // g/(r+b)
//    private static final double GREEN_MIN_G_OVER_B = 1.18;       // g/b
//
//    // WHITE (spandex-safe “sure” test)
//    private static final double WHITE_MIN_V = 0.010; // avoid calling dim noise white
//    private static final double WHITE_MAX_CHROMA_NORM = 0.29;
//    private static final double WHITE_MIN_R_OVER_G = 0.52;
//    private static final double WHITE_MAX_R_OVER_G = 0.59;
//    private static final double WHITE_MIN_G_OVER_B = 1.06;
...
//
//        double v = Math.max(r, Math.max(g, b));
//        if (v < MIN_CONFIDENCE_V) return GameColors.UNKNOWN;
//
//        // 1) Sure-white check first (very conservative)
//        if (isWhiteSure(r, g, b)) return GameColors.NONE;
//
//        // 2) Compute HSV and dominance ratios
//        HSV hsv = rgbToHsv(r, g, b);
//        double h = hsv.h, s = hsv.s;
//        double greenStrength  = g / (r + b + 1e-9);
//        double purpleStrength = b / (r + g + 1e-9);
...
//
//        // 3) GREEN
//        if (greenStrength >= GREEN_MIN_GREEN_STRENGTH ||
//                ((h >= GREEN_H_MIN && h <= GREEN_H_MAX) && s >= GREEN_MIN_S && gOverB >= GREEN_MIN_G_OVER_B)) {
//            return GameColors.GREEN;
//        }
//
//        // 4) PURPLE
//        if (purpleStrength >= PURPLE_MIN_PURPLE_STR &&
//                h >= PURPLE_H_MIN && h <= PURPLE_H_MAX && s >= PURPLE_MIN_S) {
...
//
//    // ---------- Helpers ----------
//    private static boolean isWhiteSure(double r, double g, double b) {
//        double v = Math.max(r, Math.max(g, b));
//        if (v < WHITE_MIN_V) return false;
//
//        // Normalized chroma = distance from gray axis / total intensity
//        double chromaNorm = Math.sqrt((r-g)*(r-g) + (g-b)*(g-b) + (b-r)*(b-r))
//                / (r + g + b + 1e-9);
//        if (chromaNorm > WHITE_MAX_CHROMA_NORM) return false;
//
//        double RG = r / (g + 1e-9);
//        double GB = g / (b + 1e-9);
//        if (RG < WHITE_MIN_R_OVER_G || RG > WHITE_MAX_R_OVER_G) return false;
//        if (GB < WHITE_MIN_G_OVER_B || GB > WHITE_MAX_G_OVER_B) return false;
//
//        double greenStrength  = g / (r + b + 1e-9);
//        double purpleStrength = b / (r + g + 1e-9);
//        if (greenStrength > WHITE_MAX_GREEN_STRENGTH)  return false;
//        if (purpleStrength > WHITE_MAX_PURPLE_STRENGTH) return false;
//
//        return true;
//    }
//
