...
//                .build();
        ...
        while (opModeIsActive()) {
            ...
            Actions.runBlocking(
                    new ParallelAction(
                            dropPreloadedSample,
                            robotControl.GetHighBasketActionSequence()
                    )
            );
            ...
            Actions.runBlocking(
                    new ParallelAction(
                            new InstantAction(() -> {
                                ...
                                choices.add(robotControl.GetRobotMotionValuesFromLimelightLocation(coordinates.SAMPLE1_LIMELIGHT_LOCATION));
                    ...
                    new SequentialAction(
                            new ParallelAction(
                                    new InstantAction(
                                            () -> {
                                                List<HorizontalPickupVector> choices = new ArrayList<>();
                                                choices.add(robotControl.GetRobotMotionValuesFromLimelightLocation(coordinates.SAMPLE2_LIMELIGHT_LOCATION));
                                                robotControl.SetSampleChoices(choices);
                                            }),
                                    moveToPickSample2
                            ),
            ...
            Actions.runBlocking(
                    new SequentialAction(
                            new ParallelAction(
                                    new InstantAction(() -> {
                                        List<HorizontalPickupVector> choices = new ArrayList<>();
                                        choices.add(robotControl.GetRobotMotionValuesFromLimelightLocation(coordinates.SAMPLE3_LIMELIGHT_LOCATION));
                                        robotControl.SetSampleChoices(choices);
                                    }),
                                moveToPickSample3
                            ),
