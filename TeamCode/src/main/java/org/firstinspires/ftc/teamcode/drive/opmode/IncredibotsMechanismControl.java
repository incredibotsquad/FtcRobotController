...
@Config
public class IncredibotsMechanismControl
{
    ...
    private boolean enableArmAdjustmentWithSlide = false;

    public IncredibotsMechanismControl(Gamepad gamepad, RobotHardware robotHardware) {
        gamepad2 = gamepad;
...
//        HandleManualOverride();
    ...
    private void CreateStateFromButtonPress() {
        ...
        if (gamepad2.b) {
            ...
            if (gamepad2.left_trigger > 0) {
                robotState = ROBOT_STATE.SNAP_SPECIMEN;
            }
            // right trigger + B for low basket
            else if (gamepad2.right_trigger > 0) {
                robotState = ROBOT_STATE.LOW_BASKET;
            }
        }

        if (gamepad2.x) {
            if (gamepad2.left_trigger > 0) {
                robotState = ROBOT_STATE.ARM_VERTICAL;
            }
            else if (gamepad2.right_trigger > 0) {
                robotState = ROBOT_STATE.ENTER_EXIT_SUB;
            }
        }

        if (gamepad2.y) {
            if (gamepad2.left_trigger > 0) {
                robotState = ROBOT_STATE.HANG_SPECIMEN;
            }
            else if (gamepad2.right_trigger > 0){
                robotState = ROBOT_STATE.HIGH_BASKET;
            }
        }
        ...
        if (prevArmState != ROBOT_STATE.NONE && robotState != ROBOT_STATE.NONE && prevArmState != robotState) {
            Log.i("=== INCREDIBOTS ===", "STOPPING ARMS SINCE ARM STATE CHANGE. OLD: " + prevArmState + " NEW: " + robotState);
//            robotHardware.stopSlide();
//            robotHardware.stopClawArm();
        }

        if (robotState != prevArmState) {
            Log.i("=== INCREDIBOTS ===", "NEW ARM STATE: " + robotState);
        }
    }
...
//                    Log.i("=== INCREDIBOTS ===", "PROCESSING RESTING - SLIDE FINISHED MOVING - STARTING ARM. ARM_STATE: "+ robotState);
//                }
//
...
//                    robotState = ROBOT_STATE.NONE;
//                    Log.i("=== INCREDIBOTS ===", "PROCESSING ROBOT HANG - SLIDE FINISHED MOVING - STARTING ARM. ARM_STATE: "+ robotState);
//                }
...
//                    robotState = ROBOT_STATE.NONE; //clear out state to avoid reprocessing
//                    Log.i("=== INCREDIBOTS ===", "PROCESSING ARM VERTICAL - SLIDE IS FINISHED MOVING. STARTING ARM. ARM_STATE: " + robotState);
//                }
...
//                robotHardware.operateClawServo(true);
//                robotHardware.operateWristServo(PICK_SPECIMEN_WRIST_TELE);
//                robotHardware.stopIntake();
//
//                if (!robotHardware.isSlideMotorBusy() && robotHardware.getSlidePos() != SLIDE_POSITION_RESTING) {
...
//                    robotState = ROBOT_STATE.NONE;
//                    Log.i("=== INCREDIBOTS ===", "PROCESSING PICK_SPECIMEN - SLIDE IS FINISHED MOVING - STARTING ARM. ARM_STATE: " + robotState);
//                }
...
//                    robotState = ROBOT_STATE.NONE; //clear out state to avoid reprocessing
//                    Log.i("=== INCREDIBOTS ===", "PROCESSING HANG SPECIMEN - ARM IS FINISHED MOVING. STARTING SLIDE. ARM_STATE: " + robotState);
//                }
...
//                    Log.i("=== INCREDIBOTS ===", "PROCESSING HIGH BASKET - CLAW ARM IS DONE MOVING - STARTING SLIDE. ARM_STATE: " + robotState);
//                }
//
//                if (Math.abs(DROP_SAMPLE_HIGH_SLIDE - robotHardware.getSlidePos()) < 10) {    //move wrist after moving the slide
//                    Log.i("=== INCREDIBOTS ===", "PROCESSING HIGH BASKET - ARM + SLIDE DONE MOVING - STARTING WRIST. ARM_STATE: " + robotState);
...
//                    robotState = ROBOT_STATE.NONE;
//                    Log.i("=== INCREDIBOTS ===", "PROCESSING RT/B CLAW ARM IS DONE MOVING - STARTING SLIDE. BUTTONSTATE: " + robotState);
//                }
...
//                }
                ...
                enableArmAdjustmentWithSlide = true;
                readyToDropHighSample = false;
                break;
...
//                if (Math.abs(ENTER_SUB_SLIDE - robotHardware.getSlidePos()) < 10) {    //move claw arm after slide is done moving
//                    robotState = ROBOT_STATE.ENTER_EXIT_SUB;
//                    Log.i("=== INCREDIBOTS ===", "PROCESSING  CLAW_ARM_AFTER_HIGH_SAMPLE - SLIDE IS DONE MOVING - STARTING ARM. ARMSTATE: " + robotState);
//                }
