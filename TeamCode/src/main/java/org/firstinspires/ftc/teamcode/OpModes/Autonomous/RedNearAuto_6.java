...
@Config
@Autonomous(name = "Red_Near_Auto_6", group = "Autonomous")
public class RedNearAuto_6 extends BaseAuto {
...
//        }
        ...
        Log.w("RedNearAuto_6", "total time: " + totalTime.milliseconds());
        // Cleanup
...
//        robotHardware.stopLimelight();
    ...
    private void processState() {
        ...
        if (stateTimer.milliseconds() > getTimeoutForState(currentState)) {
            Log.w("RedNearAuto_6", "State timeout! Advancing from: " + currentState);
...
//                                        .splineToLinearHeading(LAUNCH_POS, LAUNCH_POS.heading, new TranslationalVelConstraint(50), new ProfileAccelConstraint(-35, 35))
            ...
            case DRIVE_TO_INTAKE_1:
                Log.i("RedNearAuto_6", "Starting DRIVE_TO_INTAKE_1");
                runningActions.add(
                        new ParallelAction(
                                mecanumDrive.actionBuilder(mecanumDrive.localizer.getPose())
            ...
            case INTAKE_BALLS_1:
                Log.i("RedNearAuto_6", "Starting INTAKE_BALLS_1");
...
//                                new InstantAction(() -> robotHardware.setLaunchTurretPosition(TURRET_CENTERED_POSITION))
            ...
            case DRIVE_TO_INTAKE_2:
                Log.i("RedNearAuto_6", "Starting DRIVE_TO_INTAKE_2");
                runningActions.add(
                        new ParallelAction(
                                intakeSystem.getTurnOnAction(),
                                mecanumDrive.actionBuilder(mecanumDrive.localizer.getPose())
                ...
                break;
            case INTAKE_BALLS_2:
                Log.i("RedNearAuto_6", "Starting INTAKE_BALLS_2");
                if (spindex.isFull()) {
                    advanceToNextState();
                } else {
                    // Add ball check action (same as TeleOp)
            ...
            case DRIVE_TO_LAUNCH_2:
                Log.i("RedNearAuto_6", "Starting DRIVE_TO_LAUNCH_2");
                runningActions.add(
                        new ParallelAction(
                                spindex.moveToNextFullSlotAction(), //move to full slot so we don't end up spitting out a ball that we took in
                                mecanumDrive.actionBuilder(mecanumDrive.localizer.getPose())
            ...
            case LAUNCH_ALL_2:
                Log.i("RedNearAuto_6", "Starting LAUNCH_ALL_2");
                runningActions.add(
                        launchSystem.getPerformLaunchOnAllSlots()
                );
                stateTransitionInProgress = true;
            ...
            case MOVE_AWAY_FROM_LINE:
                Log.i("RedNearAuto_6", "Starting MOVE_AWAY_FROM_LINE");
                runningActions.add(
                        mecanumDrive.actionBuilder(mecanumDrive.localizer.getPose())
            ...
            case DONE:
                Log.i("RedNearAuto_6", "Auto complete!");
                break;
        }
    }
    ...
    private void advanceToNextState() {
        ...
        Log.i("RedNearAuto_6", "Advanced to state: " + currentState);
    }
